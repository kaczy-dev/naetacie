 ...
 * Senior Geospatial Graphics Architecture for NaEtacie 3D.
 */

export type SunlightMode = 'day' | 'golden_hour' | 'sunset' | 'night_cyberpunk' | 'auto';
...
/**
 * Applies dynamic 3D sunlight and building shading to an active MapLibre instance.
 */
...
export function applySunlightToMap(map: any, mode: SunlightMode): void {
  ...
  try {
    ...
    // 2. Adjust 3D building fill-extrusion colors if layer exists
    if (map.getLayer('3d-buildings')) {
      ...
      map.setPaintProperty('3d-buildings', 'fill-extrusion-opacity', preset.buildingOpacity);
    }
  } catch {
    /* Non-fatal if map style is currently reloading */
  }
}
